...
function findAllPSiblings(where) {
    ...
    for (let i = 1; i < children.length ; i++) {
        if (children[i].nodeName === 'P') {
            array.push(children[i].previousElementSibling);
        }
    }
...
function collectDOMStat(root) {
    ...
    (function fn(root) {
        for (const item of root.childNodes) {
            ...
            if (item.nodeType === 3) {
                obj.texts = obj.texts + 1 || 1;
            }
            if (item.nodeType === 1) {
                obj.tags[item.nodeName] = obj.tags[item.nodeName] + 1 || 1;
                for (const clases of item.classList) {
                    obj.classes[clases] = obj.classes[clases] + 1 || 1;
                }
            }
        }
    }(root));
