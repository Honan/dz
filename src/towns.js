...
filterInput.addEventListener('keyup', function() {
    ...
    const filteredCities = (filterInput.value !== '' ? citiesList.filter(city => isMatching(city.name, filterInput.value)) : []);

    let citiesListHTML = '';

    for (let city of filteredCities) {
        citiesListHTML += `${city.name}<br>`;
    }
    filterResult.innerHTML = citiesListHTML;

});

(function createRetry() {
    let retryBut = document.createElement('button');

    retryBut.textContent = 'Повторить';
    retryBut.id = 'retry';
    homeworkContainer.appendChild(retryBut);
    retryBut.addEventListener('click', init);
})();
